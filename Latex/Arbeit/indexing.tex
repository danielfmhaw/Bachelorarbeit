...
Dafür erstellen wir zunächst wieder die Kundentabelle (\ref{lst:create_table_kunde}) und definieren für den ersten Vergleich folgende Indizes:
...
Dazu erstellen wir erneut die Kundentabelle, aber erstellen dieses Mal nur einen Index (siehe~\ref{lst:indexing-create-combined}).
...
Zunächst einmal fällt auf, dass die Reihenfolge für die Werte mit und ohne Index sehr ähnlich ist.
Bevor wir feststellen, bei welchen Queries der Index benutzt wird, wollen wir die Abfragen aus~\ref{lst:indexing-b-tree-selects} zusammen mit den Ergebnissen näher beschreiben.

Die Abfrage \texttt{full\_match} weist die höchsten Werte auf, da der Name exakt Max Müller und das Geburtsdatum 01.01.1960 sein muss.
Bei den nachfolgenden Select-Befehlen weichen wir die Bedingungen immer weiter auf und gleichzeitig nimmt die Performance ab.
Nach \texttt{full\_match} folgt \texttt{combined\_match\_with\_range}, bei dem alle vor 1980 Geborenen einbezogen werden.
Im Anschluss kommt \texttt{range\_with\_like}, wo der Vorname nur mit "M" beginnen muss.
Danach folgt \texttt{exact\_with\_prefix}, bei dem zusätzlich zum Vorherigen nach dem Geburtsdatum aufsteigend sortiert wird.
Es folgt \texttt{skip\_columns}, die den Vornamen vollständig ignoriert.
Anschließend kommt \texttt{leftmost\_prefix}, bei dem nur nach dem Namen gefiltert wird.
Bei \texttt{column\_prefix}, wird statt des Namens „Müller“ jeder Nachname zurückgegeben, der mit „M“ beginnt.
Schließlich zeigt \texttt{range\_values} alle Namen zwischen „Müller“ und „Schulz“, während bei \texttt{not\_leftmost} ausschließlich nach dem Geburtsdatum gefiltert wird.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|l|l|l|}
        ...
        \hline
        full\_match & 0 & 7.26 & ja \\
        combined\_match\_with\_range & 11 & 6.94 & ja \\
        range\_with\_like & 39 & 5.78 & ja \\
        exact\_with\_prefix & 65 & 5.01 & ja \\
        skip\_columns & 144 & 3.71 & ja \\
        leftmost\_prefix & 250 & 3.08 & ja \\
        column\_prefix & 509 & 1.76 & ja \\
        range\_values & 1313 & 1.05 & nein \\
        not\_leftmost & 2420 & 1.00 & nein \\
        \hline
    \end{tabular}
    \vspace{3pt}
...
Wenn wir die Ergebnisse mit Index mit denen ohne vergleichen, dann sehen wir, dass die einzige Select-Query mit unterschiedlicher Positionierung \texttt{combined\_match\_with\_range} ist.
Wir können in der Tabelle~\ref{tab:indexing_b_tree_count_results} sehen, dass diejenigen Abfragen, die am wenigsten Zeilen zurückgeben, sind auch diejenigen sind, bei denen es die höchste Performance gibt.
Da die Reihenfolge in beiden Fällen fast identisch ist, sollte man meinen, dass der B-Baum-Index gar keinen Einfluss auf die Performance hat.
Dies gilt aber nur für die Reihenfolge und nicht für die tatsächlichen Werte der einzelnen Abfragen, denn wenn wir diese betrachten, sehen wir deutliche Unterschiede.
Beispielweise \texttt{full\_match} ist bei Beiden an erster Stelle, aber mit Index ist sie etwa 7--8-mal schneller als ohne Index.
Der Unterschied zwischen den Versionen ist aber nicht für alle Queries so groß.
Für \texttt{skip\_columns} liegt der Unterschied ungefähr nur bei Faktor 4.
Wir können das damit begründen, dass je mehr Spalten aus dem mehrspaltigen Index in der Query benutzt werden, desto größer ist der Faktor.
Der Faktor hat bei den Queries \texttt{range\_values} und \texttt{not\_leftmost} den Wert 1.
Das bedeutet, dass der Index keinen Einfluss auf die Performance hat, weshalb wir annehmen, dass er nicht verwendet wird.
Mithilfe des \texttt{EXPLAIN}-Operators können wir überprüfen, ob diese Annahme wirklich korrekt ist.
Tatsächlich zeigt die Analyse von \texttt{EXPLAIN}, dass die vermuteten Queries die einzigen sind, bei denen der Index nicht verwendet wird und beide sind auch die Langsamsten im Vergleich gewesen.
