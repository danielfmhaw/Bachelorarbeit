...
docker run -d --name mysql-primary --network mysql-network -e MYSQL_ROOT_PASSWORD=password -p 3307:3306 mysql:8 --server-id=1 --log-bin=mysql-bin
until docker exec mysql-primary mysqladmin ping -uroot -ppassword --silent &> /dev/null; do
  echo "Waiting for MySQL Primary to be ready..."; sleep 5
done

# Start Replica Containers
for ((i=1; i<=number_of_replicas; i++)); do
  docker run -d --name mysql-replica-${i} --network mysql-network -e MYSQL_ROOT_PASSWORD=password -p $((3307 + i)):3306 mysql:8 --server-id=$((i + 1)) --log-bin=mysql-bin --read-only=1
  until docker exec mysql-replica-${i} mysqladmin ping -uroot -ppassword --silent &> /dev/null; do
    echo "Waiting for MySQL Replica ${i} to be ready..."; sleep 5
  done
done
